...
import com.sunanang.sunwallpaper.downLoad.DownLoadPicActivity;
...
public class UnityPicActivity extends AppCompatActivity {

    private TopMiddlePopup middlePopup;
    public static int screenW, screenH;
    private ArrayList<ras_Image> picBeans = new ArrayList<>();
    ...
    private ArrayList<PicMenuBean> videoMenuList;  //菜单
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_unity_pic);
        ...
        backgroundShared = SharedUtils.getBackgroundShared(UnityPicActivity.this);
...
//                refreshlayout.finishRefresh(2000);
    ...
    private void init() {
        ...
        bmobQuery.getObject("uPEQ111F", new QueryListener<menu_table>() {
            @Override
            public void done(menu_table res_address, BmobException e) {
                if(e==null){
                    menuData = res_address.getMenu();
                    videoMenuList = JsonUtils.getPicMenuList(menuData);
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            queryData(0, STATE_REFRESH,videoMenuList.get(menuIndex).getNameEnglin(),0,null);
                        }
                    });
    ...
    private void initView() {
        ...
        layout = findViewById(R.id.ll_ad_fullscreen);
        title.setText(R.string.pic_community);
     ...
     * 初始化recyclerview
     */
    private void initRecyclerView(ArrayList<ras_Image> beans) {
        recyclerview.setItemAnimator(new DefaultItemAnimator());
        adapter = new GridAdapter(UnityPicActivity.this, null, beans, screenW, screenH, 2);
    ...
    private void setPopup(int type) {
        if (menu == null) {
            menu = new ArrayList<>();
            for (PicMenuBean b : videoMenuList) {
                menu.add(b.getName());

            }
        }

        middlePopup = new TopMiddlePopup(UnityPicActivity.this, screenW, screenH,
    ...
    private GridAdapter.OnRecyclerViewItemClickListener recyclerItemListener = new
            GridAdapter.OnRecyclerViewItemClickListener() {
                ...
                public void onItemClick(int position) {
                    Intent intent = new Intent(UnityPicActivity.this, DownLoadPicActivity.class);
                    intent.putExtra("picUrl",picBeans.get(position).getThumb());
                    intent.putExtra("picDownUrl",picBeans.get(position).getBasePicUrl());
...
//                    Toast.makeText(getApplicationContext(), "长按了  " + position, Toast.LENGTH_SHORT).show();
    ...
    private void setRefresh(final RefreshLayout refreshlayout) {
        cateIndex  = 0;
        queryData(cateIndex, STATE_REFRESH,videoMenuList.get(menuIndex).getNameEnglin(),2,refreshlayout);
    }
    ...
    private void setLoadmore(final RefreshLayout refreshlayout) {
        queryData(cateIndex, STATE_MORE,videoMenuList.get(menuIndex).getNameEnglin(),3,refreshlayout);
    }
    ...
    private void setOtherMenuData() {
        queryData(cateIndex, STATE_REFRESH,videoMenuList.get(menuIndex).getNameEnglin(),1,null);
    }
    ...
    protected void onRestart() {
        super.onRestart();
        String background = SharedUtils.getBackgroundShared(UnityPicActivity.this);
        if(!backgroundShared.equals(background)){
            findViewById(R.id.lin_base).setBackground(FileUtils2.compressBmp(this,background));
        }
    }
    ...
    private void queryData(int page, final int actionType, String category, final int type, final RefreshLayout refreshlayout) {
        ...
        BmobQuery<ras_Image> query = new BmobQuery<>();
        // 按时间降序查询
        ...
        query.findObjects(new FindListener<ras_Image>() {

            @Override
            public void done(List<ras_Image> list, BmobException e) {
                if(e==null){
                    if (list.size() > 0) {
                        ...
                        if(type == 0 || type == 1){
                            ...
                            for (ras_Image td : list) {
                                picBeans.add(td);
                            }
                        }else if(type == 2){
                            for (int i = 0; i < list.size(); i++) {
                                for (int j = 0; j < picBeans.size(); j++) {
                                    if(picBeans.get(j).getBasePicUrl().equals(list.get(i).getBasePicUrl())){
                                        break;
                                    }else {
                                        if(!picBeans.get(j).getBasePicUrl().equals(list.get(i).getBasePicUrl())){
                                            if(j == list.size()-1){
                                                picBeans.add(0,list.get(i));
                                            }
                                        }
                                    }
                                }
                    ...
                    Log.i("Sunanang2","查询失败：" + e.getMessage());
