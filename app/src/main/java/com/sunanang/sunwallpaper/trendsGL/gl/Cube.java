package com.sunanang.sunwallpaper.trendsGL.gl;
...
public class Cube {
    ...
    public void loadGLTexture(GL10 gl, Context context) {
        ...
        Bitmap bitmap = null;
        String paths = SharedUtils.getFileGLShared(context);
        if (!TextUtils.isEmpty(paths)){
            bitmap = BitmapFactory.decodeFile(paths);
        }else {
            Toast.makeText(context,R.string.bitmapError,Toast.LENGTH_SHORT).show();
            InputStream is = context.getResources().openRawResource(R.drawable.timg);
            try {
